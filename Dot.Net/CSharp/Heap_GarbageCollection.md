...
最后返回对象的引用(与this的值一致)。
**注：步骤2中描述的这些操作中的调用构造器和返回对象的引用的实际顺序，并不一定如上所述。**
...
然后，CLR检查所有的**活动根（引用类型的变量）**，查看它们引用了哪些对象，CLR会标记托管堆上所有被活动根引用的的对象，也就是将这些根上的同步块索引字段中刚刚被设置为0的那一位，设置为1。
...
包装本机资源的类的对象（可终结对象）的内存不是马上就回收，因为Finalize方法中的代码可能会访问对象中的字段。
